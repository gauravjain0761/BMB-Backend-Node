...
exports.verify_otp = async (req, res) => {
  try {
    ...
    let { contact_number, otp } = req.body; type = req.query.type;
    await doctorsModel.findOne({ contact_number: contact_number }).then(async (doc) => {
      if (!doc) {
        errorResponse(422, "The Number is not registerd.", res)
      } else {
...
exports.reset_password = async (req, res) => {
  try {
    let {contact_number, password}  =req.body; obj= {}
    await doctorsModel.findOne({ contact_number: contact_number }).then(async (docs) => {
      if (!docs) {
        errorResponse(422, "Account not registered.", res)
      } else {
